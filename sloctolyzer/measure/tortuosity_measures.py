...
import math
import numpy as np
from sloctolyzer.measure.function_ import fractal_dimension, smoothing
from sloctolyzer.measure.function_ import retipy_math as m
...
def global_cal(retina):
    ...
    # 20/03/2024 - correcting as np.unique(retina.vessel_image) = [0,1,2] somehow..
    # if np.max(vessel_)>1:
    #     vessel_=vessel_/255
...
def _curve_length(x, y, distance_measure="euclidean"):
    """
    ...
    if distance_measure == "euclidean":
        distance_funct = _distance_2p
    elif distance_measure == "manhattan":
        distance_funct = _distance_1p
    distance = 0
    for i in range(0, len(x) - 1):
        d = distance_funct(x[i], y[i], x[i + 1], y[i + 1])
        distance += d
    return distance
...
def distance_inflection_count_tortuosity(x, y):
    ...
    """
    curve_chord_ratio = distance_measure_tortuosity(x, y)
    inflection_pts = _detect_inflection_points(x, y)
    return curve_chord_ratio * (len(inflection_pts) + 1), len(inflection_pts)
...
def squared_curvature_tortuosity(x, y):
    ...
    for i in x_values:
        ...
        if (x_1 != 0) and (y_1 != 0):
            curv = (x_1*y_2 - x_2*y_1)/(y_1**2 + x_1**2)**1.5
            curvatures.append(curv)
            x_grid.append(i)
    return abs(np.trapz(curvatures, x_grid))
...
########################## evaluate_window() altered in Automorph

def evaluate_window(window: Window, min_pixels_per_vessel=10, sampling_size=6, r2_threshold=0.80):  #removed store_path='/home/jupyter/Deep_rias/Results/M2/artery_vein/vein_binary_process'# pragma: no cover
    """
    ...
    FD_binary,VD_binary,Average_width = global_cal(window)
    ...
    for i in range(0, window.shape[0], 1):
        ...
        retina = Retina(bw_window, "window{}" + window.filename, window.segmentation_path, window.resolution) # added store_path and resolution
        retina.skeletonization() # added line on 26/02/2024, so that retina.vessel_image is binary image and 
        # retina.np_image is skeleton image
