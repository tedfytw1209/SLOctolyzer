...
from sloctolyzer.measure.function_ import thinning
...
class Retina(object):
    ...
    def __init__(self, image: np.ndarray, image_path: str, store_path:str, scalex: float):
        #print('!!!',image_path)
        #print('@@@@',store_path)
        ...
        if image is None:
            ...
            self.vessel_image = image#self._open_image(self.segmentation_path) # changed 15/02/2024
            
        # commented out 15/02/2024
        # if '/' in image_path:
        ...
        # resolution_list['Scale_resolution'][resolution_list['Name']==img_name].values[0]

        # pixel scaling - added 15/02/2024 - isn't actually used??
        self.resolution = scalex
        ...
        self.old_image = None
        #self.np_image = color.rgb2gray(self.np_image) # commented out 15/02/2024
        #self.original_base64 = self.get_base64_image(self.np_image) # commented out 15/02/2024
        self.depth = 1
...
class Window(Retina):
    ...
    # Changed the window creation code to something simpler 26/02/2024
    @staticmethod
    def create_windows(x: Retina, dimension, method="separated", min_pixels=10) -> tuple:
        '''
        Given an input image, extract overlapping patches given a custom stride

        REPLACED create_windows() BELOW ON 26/02/24
        '''
        # Patching stride and patch
        x = torch.tensor(x.np_image)
        H, W = x.shape
        shape = (H - dimension + 1, W - dimension + 1, # N_strides vertically, horizontally
                 dimension, dimension) # Patch size vertically, horizontally
        H_str, W_str = x.stride(dim=-2), x.stride(dim=-1)
        strides = (H_str, W_str, H_str, W_str)
    
        # Patch the input
        x = torch.as_strided(x, size=shape, stride=strides)
        x = x[::dimension, ::dimension] # x is (batch_size, filters, patchH, patchW, patch_size, patch_size)
    
        # remove any with less than min_pixels
        x = x.reshape(-1, 1, dimension, dimension).numpy()
    
        # get positions
        N_pwidth = int(W // dimension)
        x_pos = np.array([np.array([[i*dimension,j*dimension],
                                    [(i+1)*dimension,(j+1)*dimension]]) 
                          for i in range(N_pwidth) 
                          for j in range(N_pwidth)])
    
        # Filter out empty patches below min_pixels
        x_pos = np.array([x_pos[i] for i in range(x.shape[0]) if x[i].sum() > min_pixels]) 
        x = np.array([window for window in x if window.sum() > min_pixels])
    
        return x, x_pos
    # def create_windows_OLD(
    ...
    #         windows_position = np.empty([(img_dimension // dimension) ** 2, 2, 2], dtype=np.int64) # added 64
    ...
    #                     windows[window_id, 0] = t_window[...,0] # added [...,0]
    #                     window_id += 1
    #     elif method == "combined":
    #         new_dimension = dimension // 2
    #         windows = np.empty(
    #             [(img_dimension // new_dimension) ** 2, image.depth, dimension, dimension])
    #         windows_position = np.empty([(img_dimension // new_dimension) ** 2, 2, 2], dtype=np.int64) # added 64
...
def detect_vessel_border(image: Retina, ignored_pixels=1):
    ...
    # 2024/02/27 add back in the sorting
    def vessel_extractor(window, start_x, start_y):
        """
        Extracts a vessel using adjacent points, when each point is extracted is deleted from the
        original image
        ...
        width_mask = np.zeros((window.np_image.shape))
        pending_pixels = [[start_x, start_y]]
        np_image = window.np_image 
        while pending_pixels:
            pixel = pending_pixels.pop(0)
            if np_image[pixel[0], pixel[1]] > 0:
                vessel.append(pixel)
                np_image[pixel[0], pixel[1]] = 0
        ...
        '''
        # sort by x position
        #vessel.sort(key=lambda item: item[0]) # This does not work 27/02/2024
    ...
    # # Added 29/02/2024 to sort all vessels into continous lines - removed 04/03/2024 as it's incorrect
    # if sort_vessels:
    #     contour_img = np.zeros((image.np_image.shape), dtype=np.uint8)
    #     for i, vessel in enumerate(vessels):
    #         contour_img[vessel[0], vessel[1]] = 255
    #     _, thresh = cv2.threshold(contour_img, 127, 255, 0)
    #     vessel_array, _ = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #     vessels = []
    #     for v in vessel_array:
    #         v_arr = v.squeeze(axis=1)
    #         vessels.append([list(v_arr[:,1]), list(v_arr[:,0])])
